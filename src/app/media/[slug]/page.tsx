...
const fetchSearchedData = async (
  ...
  try {
    ...
    if (selectedOptions.selectedMedia) {
      ...
      const response = await FetchApi.get(
        `https://api.themoviedb.org/3/search/multi?query=${searched}&include_adult=false&language=en-US&page=${pageNumber}`
      );
      const data = await response.json();
      return data;
    }
...
export default function MediaPage({ params }: { params: { slug: string } }) {
  ...
  const { isLoading: isRecentLoaded, data: recentData } = useQuery<any>({
    queryKey: ["recently", slug],
    queryFn: () => fetchRecentlyUpdated(slug === "tv" ? "tv" : "movie"),
    enabled: !!slug,
  });

  const { isLoading: isSearchLoading, data: searchedData } = useQuery<any>({
    queryKey: [
      "searched-data",
      search,
      currentPage,
    ...
    queryFn: () =>
      slug === "search" && search
        ? fetchSearchedData(selectedOptions, search, currentPage)
        : fetchFilteredData(selectedOptions, currentPage),
          ...
          <div className="homewrapper">
            <div className="containerHub flex gap-5 flex-col lg:flex-row">
                  ...
                  {slug === "search" && (
                    <>
                      <Filters
                        ...
                        initiallySelected={selectedOptions}
                        initiallySearch={search}
                      />{" "}
                    </>
                  )}
              ...
              <div className="min-w-full md:min-w-[376px]">
                ...
                <Recommended
                  title={slug != "recent" ? "RECENTLY UPDATED" : "NEWLY ADDED"}
                  data={
                    recentData && recentData?.length > 0
                      ? recentData?.slice(0, 10)
                      : []
                  }
                  mediaType={slug === "tv" ? "TV" : "Movie"}
                />
              </div>
            </div>
          </div>
