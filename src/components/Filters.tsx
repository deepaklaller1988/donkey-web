...
export default function Filters({handleFilters, initiallySelected, initiallySearch}:any) {
  ...
  useEffect(() => {
    if (initiallySelected) {
      if (type) {
        setSelectedType(type.filter(item => item?.code === initiallySelected.selectedMedia));
      }
      if (popularOptions) {
        setSelectedFilter(popularOptions.filter(item => item?.code === initiallySelected.selectedFilter));
      }
      if (yearsArray && yearsArray.length > 0) {
        setSelectedYear(yearsArray.filter((item:any) => item?.year == initiallySelected.selectedYear));
      }
      if (genres && genres.length > 0) {
        let genArr = initiallySelected?.selectedGenres.split(",").map((arr:any) => parseInt(arr));
        setSelectedGenre(genres.filter(item => genArr.includes(item.id)));
      }
      if (countries && countries.length > 0) {
        let genArr = initiallySelected?.selectedCountry.split(",");
        setSelectedCountry(countries?.filter((item:any)  => genArr.includes(item.iso_3166_1)));
  ...
  const handleSingleSelection = (e:any, setSelected:any) => {
    if (e.value && e.value.length > 0) {
      setSelected([e.value[e.value.length - 1]]);
    } else {
      setSelected(null);
    }
  };
  ...
  return (
    <div className="w-full my-3">
      <div className="flex flex-wrap gap-2">
        <input
          className="text-[14px] rounded-md px-3 bg-[#272727] transition text-white"
          type="text"
          placeholder="Search..."
          value={searchQuery}
        ...
        <MultiSelect
          value={selectedType}
          onChange={(e) => handleSingleSelection(e, setSelectedType)}
          options={type}
          optionLabel="name"
          placeholder="Type"
          maxSelectedLabels={1}
        ...
        <MultiSelect
          value={selectedYear}
          onChange={(e) => handleSingleSelection(e, setSelectedYear)}
          options={yearsArray}
          optionLabel="year"
          maxSelectedLabels={1}
          placeholder="Year"
        ...
        <MultiSelect
          ...
          onChange={(e) => {
            if (e.value && e.value.length > 0) {
              setSelectedFilter([e.value[e.value.length - 1]]);
            }
          }}
          options={popularOptions}
          optionLabel="name"
          ...
          className="customSelect rounded-md bg-[#272727] hover:bg-amber-500 transition"
        />
        <button className="pbgColor flex items-center rounded-full text-[#272727] transition p-2 px-4 gap-2" onClick={onSubmit}>
          <MdFilterList /> Filter
        </button>
      </div>
    </div>
