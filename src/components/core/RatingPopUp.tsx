...
const RatingPopUp = () => {
    ...
    const fetchRatingData = async () => {
        console.log(movieId,userId,ip)
        if(movieId && (userId || ip)){
        try {
            const response = await API.get(`rating/${movieId}/${userId?userId:ip}`);
    ...
    const { isLoading, error, data: ratingData, refetch } = useQuery<any>({
        ...
        <section className="bg-white/10 rounded-lg text-center p-2 px-4 flex flex-col justify-center items-center gap-2">
            ...
            <div className="text-white" style={{ width: '100px', textAlign: 'center' }}>
                ...
                <p className="text-white/50 text-sm">
                    <b className="text-sm">21</b> of{' '}
                    <b className="text-sm">10</b> (12 reviews)
                </p>
            <button onClick={handleSubmitRating} disabled={submitted} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">
                Submit Rating
            </button>
        </section>
    );
