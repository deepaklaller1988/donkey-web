...
const fetchDetails = async (movieId: number, mediaType: string) => {
  try {
    ...
    try {
      const response = await API.get(`cached/imdb-rating?mediaId=${movieId}&mediaType=${mediaType.toLowerCase()==='movie' ? 'movie' : 'tv'}`);
...
function Card({ episodeId,seasonId,movieId,index, mediaType, quality, isBookmarked = false, isMyList = false, queryClient }: any) {
